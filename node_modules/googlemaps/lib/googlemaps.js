...
exports.places = function(latlng, radius, key, callback, sensor, types, lang, name, rankby, pagetoken) {
  ...
  args.sensor = sensor || 'false';
   ...
   console.log("args in place function = "+JSON.stringify(args.location));

  return makeRequest('/maps/api/place/search/json', args, true, returnObjectFromJSON(callback));
};

// make to radar search in place 
exports.radarPlaces = function(options, callback) {

  var args = {};

  if (options.location) args.location = options.location;
  if (options.radius) args.radius = options.radius;
  ...
  args.sensor = options.sensor || 'false';


  return makeRequest('/maps/api/place/radarsearch/json', args, true, returnObjectFromJSON(callback));
};

// make to nearby search in place 
exports.nearby = function(options, callback) {

  var args = {};

  if (options.location) args.location = options.location;
  if (options.radius) args.radius = options.radius;
  if (options.rankby) args.rankby = options.rankby;
  if (options.types) args.types = options.types; 
  if (options.language)  args.language = options.language;
  if (options.name) args.name = options.name;
...
exports.placeName = function(options, callback) {
...
exports.placeDetails = function(placeid, language, callback, sensor) {
  var args = {
    placeid: placeid
  };
